...
if True: # set json variables
    with open('cert-renew-auto_ios.json') as f:
        js = json.load(f)
...
if True: # get hostname and trustpoint, check existing cert expiry date, re-enroll cert
    with ConnectHandler(**device) as channel:
        ...
        print("Checking auto-enroll status...")
        channel.send_command('\n')
        cmd3 = 'sh run | b trustpoint'     
        output = channel.send_command(cmd3)
        output_lines = output.splitlines()

        for idx, val in enumerate(output_lines):
            if 'crypto pki trustpoint' in val:
                start_index = idx
            if 'rsakeypair' in val:
                end_index = idx + 5
        
        autoenroll = False
        trustpoint_config = output_lines[start_index:end_index]
        for idx, val in enumerate(trustpoint_config):
            if 'auto-enroll' in val:
                autoenroll = trustpoint_config[idx]
        
        if autoenroll:
            autoenroll_split = autoenroll.split()
            if int(autoenroll_split[1]):
                if int(autoenroll_split[1]) > 89 and int(autoenroll_split[1]) < 95:
                    print("Trustpoint auto-enroll status confirmed...")
                elif int(autoenroll_split[1]) < 90:
                    print("TP auto-enroll configuration not standard. Fixing now...")
                    channel.send_command('\n')
                    channel.send_command_timing('config t')
                    channel.send_command_timing(f'crypto pki trustpoint {trustpoint}')
                    channel.send_command_timing('auto-enroll 92')
                    channel.send_command_timing('end')
                    channel.save_config()
                else:
                    print("There was an issue validating auto-enroll. Please check manually...")
        else:
            print("No auto-enroll configuration. Adding now...")
            channel.send_command('\n')
            channel.send_command_timing('config t')
            channel.send_command_timing(f'crypto pki trustpoint {trustpoint}')
            channel.send_command_timing('auto-enroll 92')
